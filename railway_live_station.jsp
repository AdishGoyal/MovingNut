		...
		<script>
			$(document).ready(function(){
				$("button").click(function(){
					...
					var month=$("#date2").val();
					var year=$("#date3").val();
					var url="https://api.railwayapi.com/v2/live/train/"+t_number+"/date/"+day+"-"+month+"-"+year+"/apikey/e8mf15q56w/";
					$.get(url,function(response)
					{
						...
						for(var i=0; i<response.route.length;i++)
						{
							...
							var actdep = response.route[i].actdep;
							var day  = response.route[i].day;
							var status = response.route[i].status;
							var schdep = response.route[i].schdep;
							var station=response.route[i].station.name;
							str+="<tr><td>"+i+"</td><td>"+scharr+"</td><td>"+actdep+"</td><td>"+day+"</td><td>"+status+"</td><td>"+schdep+"</td><td>"+station+"</td></tr>";
						}
...
<body>
	...
	<b>Train Number</b><input type="text" class="form-control" placeholder="Enter Train Number" id="t1"/><br>
	<b>Date</b>
		<div id="t3">
			<select id="date1">
